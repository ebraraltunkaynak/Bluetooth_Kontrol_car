...
const int sag_geri = 9;     // INPUT 2 (Pin configrasyonu)
...
pinMode(sol_ileri, OUTPUT);  //Pinlerin çıkış olarak tanımlanması
pinMode(sol_geri, OUTPUT);
pinMode(sag_enable, OUTPUT);
pinMode(sol_enable, OUTPUT);
Serial.begin(9600);                // Serial iletişimin başlatılması
}

void loop() {
if (Serial.available() > 0) {
char gelen_veri = Serial.read(); // gelen verinin değişkene atılması
...
if( gelen_veri == 'W' ) { // gelen_veri 'S' ise kendi etrafında 3 defa dönsün
/* araç dursun */
for(int i=0; i<40;i++){
